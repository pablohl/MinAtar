...
#
#####################################################################################################################
class ACNetwork_LSTM(nn.Module):
    def __init__(self, in_channels, num_actions):
        super(ACNetwork_LSTM, self).__init__()
        ...
        # Output layer:
        self.policy = nn.Linear(in_features=32, out_features=num_actions)
        self.value = nn.Linear(in_features=32, out_features=1)
        self.hidden=None
        self.cell=None
        self.reset_hidden()

    def reset_hidden(self):
        self.hidden = torch.ones(1, 1, 32) * nn.Parameter(torch.randn(1, 1, 32))
        self.cell = torch.ones(1, 1, 32) * nn.Parameter(torch.randn(1, 1, 32))
    ...
    def forward(self, x):
        ...
        # Output from the final hidden layer with derivative of sigmoid linear
      #  print(x.shape)
    ...
    #    print(f.softmax(self.policy(x), dim=1))
    #    print(self.value(x))
...
def AC_lambda(env, output_file_name, store_intermediate_result=False, load_path=None, alpha=ALPHA):
    ...
    network = ACNetwork_LSTM(in_channels, num_actions).to(device)
